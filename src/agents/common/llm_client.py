...
class OllamaClient:
    ...
    async def agenerate(self, prompt: str, max_tokens: Optional[int] = None, temperature: Optional[float] = None) -> str:
        """텍스트 생성 (비동기)"""
...
class OllamaChatWrapper:
    ...
    def create(self, messages: List[Dict[str, str]] = None, max_tokens: int = 1024, temperature: float = 0.7, **kwargs):
        response_text = self.ollama_client.chat(messages=messages, max_tokens=max_tokens, temperature=temperature)
...
class OllamaResponse:
    ...
    def __init__(self, text: str):
        ...
        self.choices = [type('Choice', (), {'message': type('Message', (), {'content': text, 'role': 'assistant'})(), 'index': 0})()]
...
def create_ollama_client(
    ...
    """Ollama 클라이언트 생성"""
    return OllamaChatWrapper(OllamaClient(model_name, "http://localhost:11434/v1", max_tokens, temperature, verbose))