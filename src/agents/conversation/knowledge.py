...
class KnowledgeAgent:
    ...
    def __init__(
        ...
        self.llm_client = LLMClient(agent_name="knowledge")
    ...
    async def process(self, state: AgentState) -> dict:
        ...
        logger.info(f"[KnowledgeAgent] Processing knowledge search for query: {query}")
        ...
        try:
            ...
            if not vector_results:
                logger.info("[KnowledgeAgent] No relevant knowledge found in Vector DB.")
                return {
                    "knowledge_results": {
                        "documents": [],
            ...
            logger.info(f"[KnowledgeAgent] Completed in {processing_time:.2f}ms. Generated {len(result.get('recommended_approaches', []))} recommendations.")
            
            return result

        except Exception as e:
            logger.error(f"[KnowledgeAgent] Error during knowledge retrieval: {str(e)}")
            return {
                "knowledge_results": {
                    "documents": [],
                    "synthesis": f"Error occurred: {str(e)}"
                },
                "recommended_approaches": []
            }
    ...
    async def _expand_query(self, query: str, domain_hints: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"[KnowledgeAgent] Expand query failed, using fallback: {str(e)}")
            return {
    ...
    async def _search_knowledge_base(self, query: str, domains: List[str]) -> List[Dict[str, Any]]:
        ...
        if not self.embedding_client or not self.pinecone_client:
            logger.warning("[KnowledgeAgent] Vector DB and Embedding client not provided.")
            return []
        ...
        try:
            ...
            logger.error(f"[KnowledgeAgent] Vector search failed: {str(e)}")
            return []
    ...
    async def _get_documents(self, doc_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[KnowledgeAgent] DB document fetch failed: {str(e)}")
            # 임시로 더미 데이터 혹은 빈 리스트 반환
            return []

    async def _assess_applicability(self, documents: List[Dict], user_context: Dict) -> Dict[str, Dict]:
        ...
        try:
            scores_list = await self.llm_client.generate_json(
                system_prompt=system_prompt,
                user_message=user_message,
            )
            # 리스트를 딕셔너리로 맵핑
            ...
            logger.warning(f"[KnowledgeAgent] Applicability assessment failed: {str(e)}")
            return {str(i+1): {"score": 0.5, "notes": "assessment failed"} for i in range(len(documents))}

    async def _synthesize_knowledge(self, query: str, documents: List[Dict], applicability: Dict) -> Dict[str, Any]:
        """조회된 문서들과 사용성 평가 결과를 바탕으로 최종 권장사항을 종합 생성합니다 (LLM)"""
        if not documents:
            return {"synthesis": "No evidence.", "recommended_approaches": [], "evidence_level": "low"}
        ...
        try:
            ...
            logger.warning(f"[KnowledgeAgent] Knowledge synthesis failed: {str(e)}")
            return {"synthesis": "Synthesis failed.", "recommended_approaches": [], "evidence_level": "low"}

    def _build_output(self, documents: List[Dict], vector_results: List[Dict], applicability: Dict, synthesis: Dict) -> Dict[str, Any]:
