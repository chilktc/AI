    ...
    def _apply_accessibility_adjustments(
        ...
        if "visual_impairment" in accessibility_needs:
            ...
            for sentence in sentences:
                if sentence in '.!?':
                    new_markers.append(TTSMarker(
                        position=position,
                        instruction="pause_short"
                    ))
        ...
        for segment in script.segments:
            ...
            if segment.segment_type in ["opening", "closing", "reflection"]:
                try:
                    ...
                    logger.warning(f"[ScriptPersonalizer] LLM personalization failed for {segment.segment_id}: {str(e)}")
...
"""
        ...
        try:
            response = await self.llm_client.generate(
                system_prompt="You are a helpful podcast script editor.",
                user_message=prompt
            )
    ...
    def _create_fallback_script(
        ...
        return PersonalizedScript(
            ...
            personalization_meta=PersonalizationMeta(
                applied_style={"fallback": True},
                adjusted_segments=[],
                attitude_applied="balanced"
            )
        )
...
async def create_script_personalizer_node(
    ...
    agent = ScriptPersonalizerAgent(
        db_client=db_client,
        enable_deep_personalization=enable_deep_personalization
    )
