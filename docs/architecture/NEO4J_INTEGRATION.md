...
4. [STORAGE_MODE — AI서버가 Neo4j에 접근하는 3가지 방식](#4-storage_mode--ai서버가-neo4j에-접근하는-3가지-방식)
...
Backend 서버 (app-3:8080)
    │
    │  AI서버의 내부 API로 요청 전달
    │  GET /internal/graph/users/{user_id}/data
    ▼
AI 서버 (app-2:8000)
    │
    │  Neo4j에 3개 Cypher 쿼리 실행:
    │
    ├─ 쿼리 1: 이 사용자의 모든 GoTNode 조회
    │    MATCH (u:User)-[:HAS_SESSION]->(s)-[:REASONED_BY]->(g:GoTNode)
    │
    ├─ 쿼리 2: 자주 연결되는 키워드 쌍 추출
    │    MATCH (g1:GoTNode)-[:LEADS_TO]->(g2) ... ORDER BY count DESC
    │
    └─ 쿼리 3: 카테고리별 분포 집계
         MATCH (g:GoTNode) RETURN g.group, count(*)
    │
    │  결과를 프론트엔드용 JSON으로 변환
    ▼
{
  "nodes": [
    { "id": "ep1_node1", "label": "의견 무시", "group": "leadership", "val": 100 },
    { "id": "ep1_node2", "label": "번아웃",   "group": "emotional_exhaustion", "val": 50 }
  ],
  "links": [
    { "source": "ep1_node1", "target": "ep1_node2", "weight": 0.8 }
  ],
  "frequent_keywords": [ { "tags": ["리더십", "번아웃"], "count": 12 } ],
...
## 4. STORAGE_MODE — AI서버가 Neo4j에 접근하는 3가지 방식

### 4-1. 왜 접근 방식이 여러 개인가?

AI서버(app-2)가 Neo4j 데이터베이스에 접근하는 방법이 3가지 있다.
이것은 **Neo4j가 어디에 설치되어 있느냐**에 따라 달라진다.

```
방식 1 (local): AI서버가 Neo4j에 직접 접속
┌────────────────────────────────────────┐
│  같은 서버 (또는 같은 네트워크)         │
│  ┌───────────┐     ┌──────────┐       │
│  │ AI 서버   │────▶│ Neo4j    │       │
...
│  └───────────┘     └──────────┘       │
└────────────────────────────────────────┘
→ 가장 빠르고 단순한 방식. 현재 코드가 이 방식 기준으로 작성됨.


방식 2 (proxy): Backend서버를 거쳐서 접속
┌───────────┐  REST  ┌───────────┐  bolt  ┌──────────┐
│ AI 서버   │───────▶│ Backend   │───────▶│ Neo4j    │
│ app-2     │        │ app-3     │        │          │
└───────────┘        └───────────┘        └──────────┘
→ AI서버에 Neo4j 연결 정보가 필요 없음. 대신 Backend팀이 중간 API를 만들어야 함.


방식 3 (hybrid): Neo4j는 직접, MySQL만 Backend 경유
┌───────────┐  bolt  ┌──────────┐
│ AI 서버   │───────▶│ Neo4j    │  ← 직접 접속
│ app-2     │        └──────────┘
│           │  REST  ┌──────────┐
│           │───────▶│ Backend  │  ← MySQL만 여기를 거침
└───────────┘        └──────────┘
→ Neo4j는 빠르게 직접 쓰되, MySQL 데이터는 Backend를 통해 안전하게 접근.
```

### 4-2. 각 모드의 상세 비교

| 항목 | `local` | `proxy` | `hybrid` |
|------|---------|---------|----------|
| **Neo4j 접근** | AI서버가 **직접** Bolt 연결 | Backend API를 **거쳐서** 접근 | AI서버가 **직접** Bolt 연결 |
| **MySQL 접근** | AI서버가 **직접** 접속 | Backend API를 **거쳐서** 접근 | Backend API를 **거쳐서** 접근 |
| **Pinecone 접근** | AI서버가 **직접** 접속 | Backend API를 **거쳐서** 접근 | AI서버가 **직접** 접속 |
| **언제 사용?** | 개발 환경, 같은 서버 | 프로덕션(DB 분리 시) | Neo4j만 같은 서버일 때 |
| **필요한 환경변수** | NEO4J_URL, USER, PASSWORD | 없음 (Backend가 처리) | NEO4J_URL, USER, PASSWORD |
| **Backend팀 추가 작업** | 없음 | `POST /api/v1/graph/query` 구현 필수 | 없음 |
| **속도** | 빠름 (1홉) | 느림 (2홉) | 빠름 (1홉) |

### 4-3. 현재 코드는 어떤 모드 기준으로 작성되었는가?

**현재 코드는 `local` / `hybrid` (직접 접속) 방식 기준으로 작성되어 있다.**

이유:
1. `podcast_reasoning.py`의 `_save_got_to_neo4j()`가 **Cypher 쿼리를 직접 실행**한다
2. `routes/graph.py`의 조회 API가 **Cypher 쿼리 3개를 직접 실행**한다
3. `proxy` 모드의 `GraphProxyClient`는 코드가 있지만, Backend 엔드포인트가 **아직 없다**
4. `.env` (로컬 개발 환경변수) 에 `STORAGE_MODE=local`로 설정되어 있다

> `config/settings.yaml`의 기본값은 `proxy`로 되어 있으나,
> 이는 프로덕션 배포를 위한 설정이다.
> **실제로 동작하는 코드는 local/hybrid의 직접 접속 방식**이다.

### 4-4. 코드에서 모드가 결정되는 과정

```python
# 1단계: 설정에서 모드 읽기 (config/loader.py)
#   - 환경변수 STORAGE_MODE가 있으면 그 값 사용
#   - 없으면 config/settings.yaml의 storage.mode 값 사용
#   - 그것도 없으면 기본값 "local"

# 2단계: 팩토리에서 클라이언트 생성 (src/db/factory.py)
def create_graph_client():
    mode = get_settings().storage_mode

    if mode == "proxy":
        # Backend API를 거치는 프록시 클라이언트 생성
        return GraphProxyClient(BackendClient())
    else:
        # "local" 또는 "hybrid" → Neo4j에 직접 접속하는 클라이언트 생성
        return Neo4jClient()

# 3단계: 에이전트나 라우트에서 사용
async with create_graph_client() as client:
    result = await client.execute_query("MATCH (g:GoTNode) RETURN g")
```
...
| **DB 접속** | Backend 프록시 | `src/db/api_proxy.py` | proxy 모드용 (코드만 완료, 사용 불가) |
...
| **조회** | 사용자 그래프 조회 API | `src/api/routes/graph.py` | Cypher 쿼리 3개 실행 |
...
| **Backend graph/query API** | 미구현 | proxy 모드 선택 시 필요 | Backend팀 |
...
├── neo4j_client.py              ← 직접 접속 (local/hybrid 모드)
├── api_proxy.py                 ← Backend 경유 (proxy 모드)
└── factory.py                   ← 모드별 자동 선택

src/api/                         ← Neo4j 데이터 가공/전송
├── graph_cumulative.py          ← 누적 그래프 EMA 계산 + RDB 저장
├── graph_transformer.py         ← GoT → 프론트엔드 JSON 변환
├── routes/graph.py              ← 그래프 조회 API (Cypher 실행)
...
### 6-4. Backend팀에 필요한 API 엔드포인트

**현재 우선순위가 높은 것** (Neo4j → RDB 누적):

| 엔드포인트 | 메서드 | 용도 | 우선순위 |
|-----------|--------|------|---------|
| `/api/v1/graph_nodes?user_id={id}` | GET | 기존 누적 데이터 조회 (Mode A) | **높음** |
| `/api/v1/graph_nodes` | PUT | 갱신된 누적 데이터 저장 (Mode A) | **높음** |
| `/api/v1/graph_analyses` | POST | GoT 분석 결과 저장 | **높음** |
| `/api/v1/graph_nodes/episodes` | POST | 에피소드별 Raw 데이터 저장 (Mode B) | 보통 |
| `/api/v1/graph/query` | POST | Cypher 쿼리 프록시 (proxy 모드 전용) | 낮음 |

### 6-5. RDB에서 누적 데이터 활용 — 쿼리 예시
...
GROUP BY gn.group_category
ORDER BY node_count DESC
LIMIT 5;

-- 예상 결과:
-- +----------------------+------------+------------+
-- | group_category       | node_count | avg_weight |
-- +----------------------+------------+------------+
...
-- 카테고리 간 연결 빈도 상위 5개
SELECT
    src.group_category  AS from_category,
    tgt.group_category  AS to_category,
    COUNT(*)            AS link_count,
    ROUND(AVG(ge.weight), 3) AS avg_link_weight
FROM graph_edges ge
JOIN graph_nodes src ON ge.source_node_id = src.id
JOIN graph_nodes tgt ON ge.target_node_id = tgt.id
WHERE ge.user_id = 'u_001'
  AND src.group_category != tgt.group_category   -- 같은 카테고리 내 제외
GROUP BY src.group_category, tgt.group_category
ORDER BY link_count DESC
LIMIT 5;

-- 예상 결과:
-- +----------------------+----------------------+------------+-----------------+
-- | from_category        | to_category          | link_count | avg_link_weight |
-- +----------------------+----------------------+------------+-----------------+
...
FROM graph_nodes gn
WHERE gn.user_id = 'u_001'
  AND gn.group_category = 'leadership'
...
- 사용자 그래프 조회 API (`routes/graph.py`)
...
| 순서 | 엔드포인트 | 용도 |
|------|----------|------|
| 1 | `GET /api/v1/graph_nodes?user_id=X` | 기존 누적 데이터 조회 |
| 2 | `PUT /api/v1/graph_nodes` | 갱신된 누적 데이터 저장 |
| 3 | `POST /api/v1/graph_analyses` | GoT 분석 결과 저장 |
| 4 | `GET /api/v1/graph/users/{id}/data` | 프론트엔드용 그래프 조회 (AI서버 프록시) |

> RDB 테이블 스키마는 섹션 6-5의 쿼리 예시를 참고.
> `graph_nodes` 테이블과 `graph_edges` 테이블이 필요하다.
...
| 3 | RDB 누적 테이블 스키마 합의 (`graph_nodes`, `graph_edges`) | 개발자3 + Backend팀 |
...
| 2-7 | `STORAGE_MODE=local` 환경변수 설정 |
...
| # | 작업 | 엔드포인트 |
|---|------|----------|
| 3-B-1 | `graph_nodes`, `graph_edges` 테이블 생성 | MySQL DDL |
...
-- 예상 결과:
-- +----------------------+----------------+-------------+------------+
-- | group_category       | affected_users | total_nodes | avg_weight |
-- +----------------------+----------------+-------------+------------+
...
# 저장소 모드
storage:
  mode: "proxy"   # local | proxy | hybrid
                   # 현재 코드는 local 기준으로 작성됨
                   # 프로덕션 배포 시 STORAGE_MODE=local 환경변수 설정 필요
...
| 변수 | 설명 | 기본값 | 필수 여부 |
|------|------|--------|---------|
| `STORAGE_MODE` | Neo4j 접근 방식 | `proxy` (yaml) / `local` (.env) | 프로덕션 시 `local` 설정 권장 |
| `NEO4J_URL` | Neo4j Bolt URI | `bolt://localhost:7687` | local/hybrid 모드 시 필수 |
| `NEO4J_USER` | Neo4j 사용자명 | `neo4j` | local/hybrid 모드 시 필수 |
| `NEO4J_PASSWORD` | Neo4j 비밀번호 | (없음) | local/hybrid 모드 시 **필수** |
...
| Backend 경유 접속 | `src/db/api_proxy.py` | `GraphProxyClient.execute_query()` |
| 모드별 자동 선택 | `src/db/factory.py` | `create_graph_client()` |
...
| 사용자 그래프 조회 | `src/api/routes/graph.py` | `get_user_graph_data()` |
