...
- 프로덕션: `.env`에 `LLM_PROVIDER=bedrock`, `BACKEND_API_URL=http://<APP3_IP>:8080/api`
...
Backend 팀에 확인:
- app-3에 `/api/{resource}` 엔드포인트가 배포되어 있는지
...
- [ ] `BackendClient` 연동 테스트 (AI → app-3:8080/api)
...
2. **팟캐스트**: `POST /api/podcasts/episodes` 테스트 요청 → 에피소드 응답 수신
