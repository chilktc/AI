...
```
GET /api/graph_nodes?user_id={uuid}&limit={n}
...
```
[프론트엔드] → GET /api/graph_nodes?user_id=xxx
...
#
# 흐름:
#   1. Backend에서 기존 누적 노드/엣지 조회 (GET /api/graph_nodes)
#   2. merge_nodes_from_got() / merge_edges_from_got()로 EMA 계산
#   3. 갱신된 데이터를 Backend에 전송 (PUT /api/graph_nodes)
#
...
_save_graph_data() ─────────────────────────────────────────
    ...
    │   └─ POST /api/graph_analyses (에피소드 스냅샷, 하위 호환)
    │
    └─ [3] publish_graph_to_rdb() → publish_graph_cumulative_mode_a()
        │
        ├─ GET /api/graph_nodes?user_id=xxx
...
│   → Backend API: GET + PUT /api/graph_nodes          │
│                                                         │
│ ■ Backend가 UPSERT (Mode B):                             │
│   publish_graph_raw_mode_b()                            │
│   → group 검증만 수행, EMA 미계산                         │
│   → Backend API: POST /api/graph_nodes/episodes      │
...
| GET `/api/graph_nodes` 엔드포인트 | 섹션 5.1 스펙 기반 구현 | 높음 |
...
| 항목 | 내용 |
|------|------|
| PUT `/api/graph_nodes` | 섹션 5.2 스펙 기반 구현 |
| 기존 행 유지 로직 | PUT에 포함되지 않은 기존 노드는 삭제하지 않고 유지 |

**Mode B 확정 시:**
| 항목 | 내용 |
|------|------|
| POST `/api/graph_nodes/episodes` | 섹션 5.3 스펙 기반 구현 |
...
```
GET /api/graph_nodes?user_id=xxx 수신
    ↓
...
향후 조직 단위 조회 확장:
   GET /api/graph_nodes?org_id=xxx
