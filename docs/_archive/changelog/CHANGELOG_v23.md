...
| # | 작업 | 출처 |
|---|------|------|
| 1-1 | 감정 추이 조회 — `/api/users/{user_id}/emotions` (PathVar)로 변경 + 응답 갱신 | Notion N5 |
| 1-2 | 에피소드 목록 — `/api/users/{user_id}/podcasts/episodes`로 변경 + 응답 갱신 | Notion N6 |
| 1-3 | 에피소드 단건 — `/api/users/{user_id}/podcasts/episodes/{episode_id}`로 변경 | Notion N7 |
...
| 1-8 | 사용자 프로필 API 섹션 추가 (`PATCH /api/users/{user_id}/profile`) | Notion N10 |
