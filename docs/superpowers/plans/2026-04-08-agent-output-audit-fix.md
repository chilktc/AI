...
CA v2.1.0 프롬프트는 sub_themes를 6개 필드 중 하나로 포함한다. 그러나 `target_duration`과 `narrative_structure`는 v2.1.0에 없으며, 코드가 기본값으로 채워 DB에 전송한다.
sub_themes는 코드 min 보장 로직 부재로 빈 배열이 통과될 수 있다.
...
| 경로 | 포함 필드 | 이유 |
|-----|---------|-----|
| AgentState (`content_analysis`) | 6개: `user_summary`, `main_theme`, `emotional_journey`, `key_messages`, `depth_level`, `sub_themes` | v2.1.0 실제 출력 스펙. sub_themes는 min 보장으로 3개 이상 유지 |
| DB 저장 (`_build_db_payload`) | 6개 + `target_duration` 기본값, `narrative_structure` 기본값 | 백엔드 테이블 컬럼 유지 (backward compat) |

`sub_themes`는 이제 AgentState에도 실제 값(3개 이상)이 포함된다. DB 페이로드도 동일한 실제 값을 전송한다.
...
| `sub_themes` | 실제 값 3개 이상 — CA v2.1.0 min 보장 후 | ✅ 실제 값 전송 (NOT NULL 호환) |
...
코드가 기본값(4, "reflection")으로 강제 추가한다. DB 테이블 하위 호환성을 위해 이 동작은 유지한다.
...
1. **CA-1 수정**: `corrected = dict(analysis)` 전체 복사 방식을 버리고, 허용 필드 **6개**만 명시적으로 추출한다. 각 필드의 타입도 직접 검증한다. (v2.1.0의 실제 필드: user_summary, main_theme, emotional_journey, key_messages, depth_level, sub_themes)

2. **CA-2 수정**: `_validate_and_correct()`에 sub_themes **최소 개수 보장 로직을 추가**한다. LLM이 빈 배열을 반환하거나 필드를 생략하면 Script Generator에서 빈 에피소드가 생성된다. min_sub_themes 미달 시 LLM 재시도 또는 의미 있는 기본값을 반환하도록 강화한다.

**DB 저장 경로**: `target_duration`과 `narrative_structure`는 v2.1.0 프롬프트가 출력하지 않으므로 코드가 기본값을 추가하는 현재 방식 유지. `sub_themes`는 이제 AgentState에도 실제 값이 들어간다.
- AgentState용 딕셔너리 (`_validate_and_correct` 반환값): **6개 필드** (sub_themes 포함)
- DB 저장용 딕셔너리 (`_build_db_payload` 새 메서드): 6개 + `target_duration`, `narrative_structure` 기본값

---

### 왜 이렇게 하는가?

경로를 분리하는 이유는 **하위 호환성(backward compatibility)** 때문이다. DB 테이블은 백엔드 팀이 관리한다. `target_duration`이나 `narrative_structure`처럼 v2.1.0 프롬프트가 출력하지 않는 필드를 DB에 안 보내면 백엔드 INSERT가 실패할 수 있다. API 테스트가 진행 중인 지금, 그 리스크를 감수할 수 없다. 그래서:
- AgentState(파이프라인 내부 흐름): **6개 필드** — v2.1.0이 실제 생성하는 값만 포함 (sub_themes는 min 보장됨)
- DB 저장(백엔드 통신): 6개 필드 + `target_duration`/`narrative_structure` 기본값으로 기존 컬럼 구조 유지

---

### 수정 후 얻는 결과

- `content_analysis`에는 항상 정확한 타입으로 **6개 필드**가 존재한다.
- Script Generator가 `content_analysis.user_summary`를 읽을 때 항상 딕셔너리임이 보장된다.
- `content_analysis.sub_themes`는 min_sub_themes(=3) 보장으로 **항상 3개 이상의 배열**이 된다.
- DB에는 기존과 동일한 구조로 데이터가 전송되어 API 테스트에 영향 없다.

**Content Analyzer v2.1.0 프롬프트 (실제 출력, 6개 필드):**
```
...
depth_level       → "shallow"|"medium"|"deep"  (코드가 clamp 처리)
sub_themes        → list[str] (3-5개, min 보장 — CA-2 수정 후)
```

**DB 저장 영향:** `content_analyses` 테이블 — AgentState와 DB 경로를 분리

- **AgentState** (`content_analysis` 키): 6개 필드 (`_validate_and_correct` 반환값, sub_themes min 보장)
- **DB 페이로드** (`_build_db_payload` 경유): 6개 필드 + `target_duration`=4 기본값, `narrative_structure`=`"reflection"` 기본값 포함
...
```python
    def _validate_and_correct(self, analysis: dict[str, Any], depth_level: str) -> dict[str, Any]:
        ...
        반환값은 AgentState용 6개 필드만 포함한다 (v2.1.0 실제 출력 스펙).
        DB 저장 시에는 _build_db_payload()로 레거시 필드(target_duration, narrative_structure)를 추가한다.

        허용 필드 (6개): user_summary, main_theme, emotional_journey, key_messages, depth_level, sub_themes
        제외 필드 (AgentState 제외, DB에는 기본값 유지): target_duration, narrative_structure
        """
        ...
        # 6. depth_level — LLM 값 무시, complexity_score 기반으로 코드가 직접 결정
        return {
    ...
    def _build_db_payload(self, validated: dict[str, Any]) -> dict[str, Any]:
        ...
        CA v2.1.0은 target_duration, narrative_structure를 생성하지 않는다.
        그러나 content_analyses 테이블이 이 컬럼들을 보유할 수 있으므로 기본값을 포함시킨다.
        sub_themes는 _validate_and_correct()에서 min 보장된 실제 값을 그대로 사용한다.
        """
        return {
            **validated,
            "target_duration": self.min_duration,  # CA v2.1.0 미출력 — DB용 기본값
            "narrative_structure": "reflection",   # CA v2.1.0 미출력 — DB용 기본값
        }
```
...
```python
        ...
        # AgentState: v2.1.0 6개 필드 (sub_themes min 보장 포함)
        return {"content_analysis": validated_analysis}
```

- [ ] **Step 4: 기존 테스트 업데이트**

`test_content_analyzer.py` 에서 `narrative_structure`/`target_duration` AgentState 검증 줄 수정:
```python
# 수정 대상 (v2.1.0은 target_duration/narrative_structure AgentState 미포함):
# assert result["content_analysis"]["narrative_structure"] == "reflection"  → 삭제
# assert result["content_analysis"]["target_duration"] == 4  → 삭제
# sub_themes는 이제 AgentState에 포함 → 검증 유지
# assert "sub_themes" in result["content_analysis"]  → 추가
```

`mock_llm_response` fixture를 v2.1.0 형식으로 교체 (sub_themes 3개 이상 포함, target_duration/narrative_structure 제거):
```python
@pytest.fixture
def mock_llm_response() -> dict[str, Any]:
    """LLM v2.1.0이 반환할 모의 분석 결과 (6개 필드)."""
    return {
        ...
        "depth_level": "moderate",
        "sub_themes": ["직장 스트레스", "감정 조절", "자기돌봄"],  # v2.1.0 필수 포함
    }
```
...
| 리소스 | 협의 여부 | 근거 |
|--------|---------|------|
| `content_analyses` | **⚠️ 협의 필수** | 로컬 스키마 미정의. sub_themes는 이제 실제 값(3개 이상) 전송. target_duration/narrative_structure는 기본값으로 유지. 백엔드 컬럼 구조 확인 필요 |
