...
> **⚠️ trace_id API 스펙 불일치 주의**: 서브에이전트 점검 결과, `docs/architecture/API_ENDPOINTS_INTERNAL.md`의 content_analyses 저장 요청 필드에 `trace_id`가 명시되지 않았다. 백엔드 계약 JSON에는 포함되어 있으므로, **구현 전 백엔드 팀에 `trace_id` 포함 여부 최종 확인 필요**.

**에러 경로 fallback 수정 필요** (`content_analyzer.py:88`):
```python
# 현재 에러 경로 fallback (line 88 근처) — v2.1.x 이하 3-키 구조
"emotional_journey": {"opening": "", "development": "", "resolution": ""}

# 수정 후 (v2.2.0 4-키 구조로 통일)
"emotional_journey": {"opening": "", "development": "", "climax": "", "closing": ""}
```
에러 경로도 정상 경로와 동일한 4-키 구조를 출력해야 하위 에이전트(SG, SP)에서 타입 오류가 발생하지 않는다.
...
line 208-212 — `_generate_title()` `resolution_emotion` 참조 수정 (CA-3 연쇄):
```python
# Before (line 208-212)
...
| `src/agents/podcast/content_analyzer.py` | line 88 | 에러 경로 fallback: `{"emotional_journey": {"opening":"", "development":"", "resolution":""}}` | 4-키 `climax/closing` 구조로 변경 | **Task 4 Step 3** |
| `src/agents/podcast/script_generator.py` | line 208-212 (`_generate_title`) | `resolution_emotion = .get("resolution", .get("resolution_emotion", "None"))` | `.get("closing", .get("resolution_emotion", ""))` | **Task 6** |
| `src/agents/podcast/script_generator.py` | line 211 (개별 참조) | `.get("resolution", ...)` | `.get("closing", ...)` | **Task 6** (위와 동일 수정) |
| `src/agents/podcast/script_personalizer.py` | lines 141-143, 573 | `resolution` 참조 + `EmotionalJourney.resolution` | `closing` 참조 + `EmotionalJourney.closing` | **Task 9** |
...
| `docs/architecture/AGENT_OUTPUT_CONTRACT.md` | line 177 | `emotional_journey: {opening, development, resolution}` | `{opening, development, climax, closing}` | **문서 업데이트 (구현 후)** |
