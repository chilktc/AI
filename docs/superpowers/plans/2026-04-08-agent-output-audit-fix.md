...
### sub_themes는 어디서 오는가?
Content Analyzer v1.x → sub_themes를 직접 출력했음
Content Analyzer v2.1.0 → sub_themes 제거, **Podcast Reasoning(GoT)이 생성**하도록 이관

**현재 문제**: Podcast Reasoning의 GoT가 `got_result.sub_themes`를 만들지만,
`reasoning_result` 최상위 레벨로 올리는 코드가 없다.
Script Generator는 여전히 `content_analysis.sub_themes`를 읽는데, v2.1.0에선 이 값이 없다.
→ 결과: **빈 값([]) 참조 발생** — 이 계획에서 수정한다.
...
| 이슈 | 에이전트 | 제외 이유 |
|------|---------|---------|
| EM-1 | Episode Memory | Pinecone 연동 완료 시 함께 처리 |
...
> Task 4에서는 이 두 경로를 분리한다 — AgentState는 5개 필드만, DB 저장은 레거시 필드 포함.
...
`main_theme`, **`sub_themes`** (=`[]`), `emotional_journey`, **`target_duration`** (=`4`), **`narrative_structure`** (=`"reflection"`), `key_messages`, `depth_level`

CA v2.1.0 이후 LLM은 `sub_themes`, `target_duration`, `narrative_structure`를 생성하지 않는다.
하지만 코드가 여전히 기본값으로 채워 DB에 전송하고 있다.
...
**AgentState**와 **DB 저장** 경로를 분리한다:

| 경로 | 포함 필드 | 이유 |
|-----|---------|-----|
| AgentState (`content_analysis`) | 5개: `user_summary`, `main_theme`, `emotional_journey`, `key_messages`, `depth_level` | v2.1.0 스펙 준수. 하위 에이전트는 이 5개만 참조 |
| DB 저장 (`_build_db_payload`) | 5개 + `sub_themes`=`[]`, `target_duration` 기본값, `narrative_structure` 기본값 | 백엔드 테이블 컬럼 유지 (backward compat) |

`sub_themes`는 DB 페이로드에 **빈 배열로 유지**한다. 실제 sub_themes 값은 Podcast Reasoning GoT가 생성하며 `reasoning_result.sub_themes`를 통해 파이프라인 내에서 전달된다 (Task 5).
...
| `emotional_journey` | `{"opening": "...", "development": "...", "resolution": "..."}` | 없음 |
| `key_messages` | 문자열 배열 최대 5개 | 없음 |
| `depth_level` | `"deep"` / `"moderate"` / `"light"` | 없음 |
| `sub_themes` | `[]` (빈 배열) — CA v2.1.0 미출력 | ⚠️ NOT NULL이면 문제 없음 (빈 배열 전송) |
| `target_duration` | 기본값 전송 | 없음 |
| `narrative_structure` | 기본값 전송 | 없음 |
...
TIER 1: [Task 2] SA-1+SA-2    — Safety 출력 명시화
        ...
        [Task 4] CA-1+CA-2     — Content Analyzer v2.1.0 whitelist 적용
TIER 1→2 연결:
        [Task 5] PR 수정        — sub_themes 상위 노출 (GoT→reasoning_result)
TIER 2: [Task 6] SG-1+SG-2    — Script Generator sub_themes 소스 수정 + 하드코딩 제거
        [Task 7] VI-1+VI-2     — Visualization 출력 검증
TIER 3: [Task 8] BV-1+BV-2    — Batch Validator 명시 필드 추출
TIER 4: [Task 9] SP-1+SP-2    — Script Personalizer 감정 필드 참조 정정
```

> **실행 순서**: Task 1 건너뜀 → Task 2·3·4·5·7·8 (병렬 가능) → Task 6·9 (Task 4·5 완료 후)
...
## Task 4: CA-1 + CA-2 — Content Analyzer v2.1.0 whitelist 적용 [TIER 1]
...
**왜 이게 나쁜가?** 코드가 "이 5개 필드만 사용한다"고 보장해주지 않으면, 나중에 누군가 "여기서 무슨 값이 나오는지"를 코드만 보고 알 수가 없다. 필드가 존재한다는 보장이 없는 것이다.

---

**CA-2 문제: 프롬프트는 이미 제거했는데 코드가 아직 기본값으로 채운다**

> **중요 (2026-04-08 점검 확인):** 프롬프트 v2.1.0은 이미 올바르게 수정되어 있다.
> `prompts/podcast/content_analyzer.yaml` v2.1.0 line 145:
> *"아래 5개 필드만 포함하세요. 그 외 필드(sub_themes, target_duration, narrative_structure 등)는 절대 포함하지 마세요."*
>
> 따라서 **프롬프트 수정은 불필요하다. 코드만 수정하면 된다.**

현재 코드 (`content_analyzer.py:220-238`):
```python
corrected["sub_themes"] = sub_themes          # LLM이 안 줘도 [] 로 강제 추가
corrected["target_duration"] = target_duration  # LLM이 안 줘도 4 로 강제 추가
corrected["narrative_structure"] = narrative   # LLM이 안 줘도 "reflection" 으로 강제 추가
```

LLM v2.1.0은 이 3개 필드를 아예 출력하지 않는다. 그런데 코드가 "LLM이 안 줬을 때를 대비해서" 기본값으로 채워 반환한다. 그 결과:
- `content_analysis.sub_themes`는 항상 `[]` (빈 배열)
- Script Generator가 `content_analysis.sub_themes`를 읽으면 항상 빈 배열만 받는다
- 팟캐스트 에피소드에 세부 주제가 하나도 없는 상태로 생성된다
...
1. **CA-1 수정**: `corrected = dict(analysis)` 전체 복사 방식을 버리고, 허용 필드 5개만 명시적으로 추출한다. 각 필드의 타입도 직접 검증한다.

2. **CA-2 수정**: `sub_themes`, `target_duration`, `narrative_structure`를 강제 추가하는 코드 3줄을 삭제한다. 이 코드가 없어져야 AgentState에 이 필드들이 들어가지 않는다.

**단, DB 저장은 별도로 처리한다.** 현재 코드는 AgentState에 저장하는 딕셔너리를 그대로 DB에도 보낸다. 만약 DB 테이블에 `sub_themes` 컬럼이 있다면, 우리가 `sub_themes`를 안 보내는 순간 DB INSERT 오류가 날 수 있다. 그래서:
- AgentState용 딕셔너리 (`_validate_and_correct` 반환값): 5개 필드만
- DB 저장용 딕셔너리 (`_build_db_payload` 새 메서드): 5개 + 레거시 3개(기본값 포함)

---

### 왜 이렇게 하는가?

경로를 분리하는 이유는 **하위 호환성(backward compatibility)** 때문이다. DB 테이블은 백엔드 팀이 관리한다. 우리가 갑자기 `sub_themes` 필드를 안 보내면 백엔드 DB INSERT가 실패할 수 있다. API 테스트가 진행 중인 지금, 그 리스크를 감수할 수 없다. 그래서:
- AgentState(파이프라인 내부 흐름)는 깨끗하게 5개만 사용
- DB 저장(백엔드 통신)은 기존 컬럼 구조를 유지하면서 빈 기본값을 보냄

---

### 수정 후 얻는 결과

- `content_analysis`에는 항상 정확한 타입으로 5개 필드만 존재한다.
- Script Generator가 `content_analysis.user_summary`를 읽을 때 항상 딕셔너리임이 보장된다.
- `content_analysis.sub_themes`는 이제 AgentState에 없다 → Script Generator는 Task 5에서 다른 경로로 sub_themes를 받는다.
- DB에는 기존과 동일한 구조로 데이터가 전송되어 API 테스트에 영향 없다.

**Content Analyzer v2.1.0 프롬프트 (설정된 출력, 5개 필드만):**
```
user_summary      → {keywords: list, summary: str}
main_theme        → str
emotional_journey → {opening: str, development: str, resolution: str}
key_messages      → list[str] (3-5개)
depth_level       → "shallow"|"medium"|"deep"  (코드가 직접 결정)
```

**DB 저장 영향:** `content_analyses` 테이블 — AgentState와 DB 경로를 분리

- **AgentState** (`content_analysis` 키): 5개 필드만 (`_validate_and_correct` 반환값)
- **DB 페이로드** (`_build_db_payload` 경유): 5개 필드 + `sub_themes`=`[]`, `target_duration`=`0`, `narrative_structure`=`""` 기본값 포함
- → `sub_themes`는 DB에 **빈 배열로 유지** (실제 값은 Task 5에서 결정)
...
```python
# === 8. v2.1.0 출력 필드 화이트리스트 ===


def test_validate_and_correct_excludes_v1x_legacy_fields(
    agent: ContentAnalyzerAgent,
) -> None:
    """_validate_and_correct가 v1.x 레거시 필드를 결과에 포함하지 않는다 (CA-2).

    v2.1.0 프롬프트는 sub_themes/target_duration/narrative_structure 출력을 금지함.
    """
    analysis = {
    ...
    assert "sub_themes" not in result, "sub_themes는 v2.1.0 AgentState 출력 아님"
    assert "target_duration" not in result, "target_duration은 v2.1.0 AgentState 출력 아님"
    assert "narrative_structure" not in result, "narrative_structure는 v2.1.0 AgentState 출력 아님"
...
def test_build_db_payload_includes_legacy_fields(
    ...
    """_build_db_payload가 content_analyses DB용 레거시 필드를 빈 기본값으로 포함한다.

    content_analyses 테이블은 sub_themes, target_duration, narrative_structure 컬럼을
    보유할 수 있다. CA v2.1.0에서 이 값들이 생성되지 않으므로 기본값으로 채워 전송한다.
    실제 sub_themes는 Podcast Reasoning GoT에서 생성되어 reasoning_result로 전달된다.
    """
    validated = {
        "user_summary": {"keywords": ["스트레스"], "summary": "힘든 사용자"},
        "main_theme": "스트레스 관리",
    ...
    assert "sub_themes" in db_payload, "content_analyses DB에 sub_themes 컬럼 유지"
    assert db_payload["sub_themes"] == [], "CA v2.1.0 미출력 → 빈 배열"
...
def test_validate_and_correct_validates_emotional_journey_type(
    ...
    """emotional_journey가 dict 아닐 때 빈 구조로 보정한다 (CA-1)."""
    analysis = {
    ...
    assert set(ej.keys()) == {"opening", "development", "resolution"}
...
```bash
pytest tests/agents/podcast/test_content_analyzer.py::test_validate_and_correct_excludes_v1x_legacy_fields -v -m "not live"
```

Expected: **FAIL** — `sub_themes`, `target_duration`, `narrative_structure` 포함됨
...
```python
    def _validate_and_correct(self, analysis: dict[str, Any], depth_level: str) -> dict[str, Any]:
        """
        LLM 분석 결과를 v2.1.0 화이트리스트 기준으로 검증·추출한다.

        반환값은 AgentState용 5개 필드만 포함한다.
        DB 저장 시에는 _build_db_payload()로 레거시 필드를 추가한다.

        허용 필드 (5개): user_summary, main_theme, emotional_journey, key_messages, depth_level
        제외 필드 (AgentState 제외, DB에는 기본값 유지): sub_themes, target_duration, narrative_structure
        """
        ...
        # 3. emotional_journey — {opening, development, resolution} 구조 강제
        ...
        # 5. depth_level — LLM 값 무시, complexity_score 기반으로 코드가 직접 결정
        return {
    ...
    def _build_db_payload(self, validated: dict[str, Any]) -> dict[str, Any]:
        """
        content_analyses DB 저장용 페이로드를 구성한다.

        CA v2.1.0은 sub_themes, target_duration, narrative_structure를 생성하지 않는다.
        그러나 content_analyses 테이블이 이 컬럼들을 보유할 수 있으므로 기본값을 포함시킨다.
        실제 sub_themes는 Podcast Reasoning GoT가 생성 → reasoning_result.sub_themes로 전달.
        """
        return {
            **validated,
            "sub_themes": [],           # CA v2.1.0 미출력 — 실제 값은 PR GoT에서 생성
            "target_duration": 0,       # CA v2.1.0 미출력 — 미사용 (0=미정)
            "narrative_structure": "",  # CA v2.1.0 미출력 — 미사용
        }
```
...
```python
        ...
        # DB 저장: 레거시 필드 포함 (content_analyses 테이블 backward compat)
        db_payload = self._build_db_payload(validated_analysis)
        await publisher.publish(
        ...
        # AgentState: v2.1.0 5개 필드만
        return {"content_analysis": validated_analysis}
```

- [ ] **Step 4: 기존 테스트 업데이트**

`test_content_analyzer.py` 에서 `narrative_structure` 검증 줄 제거:
```python
# 삭제 대상 (v2.1.0은 narrative_structure 미출력):
# assert result["content_analysis"]["narrative_structure"] == "reflection"
```

`mock_llm_response` fixture를 v2.1.0 형식으로 교체 (sub_themes/target_duration/narrative_structure 제거, user_summary/emotional_journey 추가):
```python
@pytest.fixture
def mock_llm_response() -> dict[str, Any]:
    """LLM v2.1.0이 반환할 모의 분석 결과 (5개 필드)."""
    return {
...
- [ ] **Step 6: DB 저장 변경 사항 확인**

```bash
grep -n "publisher\|publish" src/agents/podcast/content_analyzer.py
```

`validated_analysis` (= `_validate_and_correct` 반환값)가 그대로 publish되는지 확인.
이 수정 이후 DB에는 `sub_themes`, `target_duration`, `narrative_structure`가 전송되지 않는다.
**구현 전 백엔드 팀 협의 필수** (계획서 상단 DB 협의 목록 참조).

- [ ] **Step 7: 커밋**

```bash
git add src/agents/podcast/content_analyzer.py tests/agents/podcast/test_content_analyzer.py
git commit -m "fix(content_analyzer): CA-1/CA-2 — v2.1.0 whitelist 5개 필드, 레거시 제거"
```
...
CA v2.1.0 이전에는 Content Analyzer가 이 목록을 만들어 줬다. 그런데 v2.1.0 프롬프트에서 제거되었고, 코드도 Task 4에서 AgentState에서 빼기로 했다. 그 결과 Script Generator가 `content_analysis.sub_themes`를 읽으면 항상 없거나 빈 배열을 받는다.
...
> **결정 필요**: 아래 두 가지 방법 중 하나를 선택해야 한다.
...
**선택지 B: Content Analyzer(CA)에서 sub_themes를 다시 생성**

CA 프롬프트를 v2.2.0으로 업그레이드해서 sub_themes를 다시 포함시킨다. 그러면 `content_analysis.sub_themes`가 다시 채워지고, Script Generator 코드는 아무것도 바꾸지 않아도 된다.

해야 할 일:
1. `prompts/podcast/content_analyzer.yaml`에 v2.2.0 버전을 추가 (sub_themes 복원)
2. `_validate_and_correct`에서 sub_themes 검증 코드는 유지 (Task 4에서 삭제하지 않음)
3. Script Generator 수정 불필요

- **장점**: Script Generator 수정이 필요 없다. sub_themes가 항상 채워진다.
- **단점**: GoT보다 단순한 분석이라 주제 품질이 낮을 수 있다. 프롬프트 버전을 하나 더 만들어야 한다.

---

### 현재 계획 (선택지 A 기준으로 작성됨)

아래 구현 단계는 **선택지 A**를 기준으로 작성되어 있다. 선택지 B로 변경 시 Step 3과 Task 6을 수정해야 한다.
...
현재: `content_analysis.get("sub_themes", ...)` → CA v2.1.0 이후 항상 빈 값
수정: `reasoning_result.get("sub_themes", [])` → PR이 생성한 값 사용 (Task 5 완료 후 유효)
...
**Task 4 완료 후 content_analysis.emotional_journey 구조:**
```python
{
  "opening":     "시작 감정 상태",
  "development": "감정 변화 과정",
  "resolution":  "도달할 감정 상태",
}
```
→ `start_emotion`, `resolution_emotion` 폴백은 영구적으로 `None` 반환 (데드 코드)

**선행 조건:** Task 4(CA-1/CA-2) 완료 후 실행.
`content_analysis.emotional_journey`가 `{opening, development, resolution}` 구조를 보장한 뒤 SP 수정.
...
def test_personalizer_no_v1x_emotion_field_names() -> None:
    """start_emotion/resolution_emotion v1.x 레거시 필드명 없다 (SP-2)."""
    import inspect
    from src.agents.podcast.script_personalizer import ScriptPersonalizerAgent
            ...
            if emotional_journey_data and isinstance(emotional_journey_data, dict):
                try:
                    emotional_journey = EmotionalJourney(
                        ...
                        resolution=emotional_journey_data.get("resolution", "따뜻함"),
                        # ↑ start_emotion/resolution_emotion 레거시 폴백 제거
...
git commit -m "fix(script_personalizer): SP-1/SP-2 — v1.x 레거시 필드 제거, v2.1.0 opening/resolution 사용"
```
...
| SP-1 | 4 | C | 9 | ✅ |
| SP-2 | 4 | B | 9 | ✅ |
| EM-1 | — | B | 제외 | Pinecone 선행 |
| KA-1 | — | C | 제외 | Pinecone 선행 |
| PR-1 | — | C | 제외 | Pinecone 선행 |

### 2. 빈 값 참조 해소 확인

| 에이전트 | 빈 값 참조 문제 | 해소 방법 |
|---------|-------------|---------|
| SG: `sub_themes` | CA v2.1.0 이후 항상 [] | Task 5: PR→reasoning_result.sub_themes, Task 6: SG가 PR에서 읽도록 변경 |
...
| 리소스 | 협의 여부 | 근거 |
|--------|---------|------|
| `content_analyses` | **⚠️ 협의 필수** | 로컬 스키마 미정의. sub_themes는 DB 페이로드에 `[]`로 유지 (`_build_db_payload`). 백엔드 컬럼 구조 확인 필요 |
