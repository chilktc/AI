...
| **수정** | `dev/local_db/mysql/init.sql` | `podcast_episodes` 감정+텍스트 컬럼 추가, `podcast_segments` 테이블 **제거**, `episode_summaries` 테이블 추가, `content_analyses` 테이블 추가 | Task 8, 9, 11 |
...
```python
@pytest.mark.asyncio
async def test_save_core_data_stores_emotion_and_episode_text_no_segments(mocker):
    """_save_core_data()가 감정+episode_text를 저장하고 segments를 별도 저장하지 않아야 한다."""
    mock_save = mocker.AsyncMock(return_value={"success": True, "id": "ep_1"})
    mocker.patch("src.api.routes.podcasts.BackendClient.save", mock_save)
    state = {
        "final_output": '{"episode_id":"ep_1","episode_title":"테스트","total_duration":5,"segments":[{"segment_id":"s1","script_text":"안녕하세요","segment_type":"opening","duration_minutes":1,"word_count":3,"emotional_tone":"warm","tts_markers":[]}],"key_insights":[],"themes":[]}',
        "emotion_vectors": {
    ...
    await _save_core_data(state, mock_save)

    call_args = mock_save.call_args_list
    # podcast_episodes 저장 확인
    episode_call = next(c for c in call_args if "podcast_episodes" in str(c))
    data = episode_call[1]["request"].data
...
```python
# ① podcast_episodes 저장 dict에 3개 필드 추가
episode_data = {
    # ... 기존 필드 유지 ...
...
# ② segments_data 구성 코드 전체 삭제 (routes/podcasts.py:161-178)
# ③ podcast_segments SaveRequest / backend_client.save() 호출 삭제
```
