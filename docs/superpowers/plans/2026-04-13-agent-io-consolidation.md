...
| `_save_core_data()` 확장 | emotion_vectors + memory_text를 podcast_episodes에 함께 저장 | Task 8 |
...
[TIER 1 병렬 실행 중...]
    ↓
ContentAnalyzer 완료
    → publisher.publish("content_analyses", {user_summary: {keywords, summary}})
    → SSE: content_analyzer_complete 이벤트
    → ✅ 1차 로딩 완료 → 프론트: 안내화면 표시 (화면 1)
    ↓
[TIER 2~4 계속 실행 중... 사용자는 화면 1을 보고 있음]
    ↓
사용자: "다음" 버튼 클릭 → 2차 로딩 시작
    ↓
TIER 4 완료 → _save_core_data() → SSE: result 이벤트
    → ✅ 2차 로딩 완료 → 프론트: 에피소드 결과 화면 (화면 2)
```
...
| **화면 2 (2차 로딩 후)** | 타이틀, 이미지, 에피소드 텍스트, 감정 키워드 2~3 | `podcast_episodes` + `podcast_segments` + `visualizations` | TIER 4 완료 후 |
...
💾 **`_save_core_data()`** (동기 — 응답 반환 전) → `podcast_episodes`(+감정컬럼) + `podcast_segments` · `visualizations` *(Task 8 확장)*
...
> `result` SSE 이벤트 수신 → 프론트가 Backend GET API로 상세 조회 후 렌더링
...
| **최종 내용** | `GET /podcast_episodes/{episode_id}` | `segments[].script_text` |
...
```
> ✏️ **변경 이유:** `content_analyses` 테이블은 **화면 1(안내화면) 전용 조기 저장**으로 활용. TIER 1이 완료되는 시점에 `user_summary`를 즉시 저장하여 Backend가 `content_ready` SSE 이벤트를 발행할 수 있게 함. `main_theme`, `sub_themes` 등 나머지 분석 필드는 내부 파이프라인 전용 — 저장 불필요.
...
| routes/podcasts.py | podcast_episodes | RESOURCE_PODCAST_EPISODE | TIER 4 완료 후 | **동기** | ✅ **화면 2** (Task 8 확장: +감정컬럼 +episode_text) |
...
> ContentAnalyzer 완료 → `publisher.publish()` → `content_ready` SSE → `GET /content_analyses/{id}` 조회

| 화면 요소 | DB 컬럼 | AgentState 출처 |
|---------|--------|---------------|
| **감정 키워드 태그** | `content_analyses.data.user_summary.keywords` | `content_analysis.user_summary.keywords` |
| **공감 안내 메시지** | `content_analyses.data.user_summary.summary` | `content_analysis.user_summary.summary` |
...
> 파이프라인 완료 → `_save_core_data()` 자동 저장 → `result` SSE → `GET /podcast_episodes/{episode_id}` 조회
...
2. `podcast_episodes`에 `episode_text TEXT` 추가 — 화면 2 에피소드 전문 표시 (segments → memory_text 형태)
3. `podcast_segments`에 `user_id`, `session_id` 추가 — 유저 데이터 격리 보장
4. `_save_core_data()`에서 감정 데이터 + memory_text를 podcast_episodes에 함께 저장
...
- [ ] **Step 2: init.sql — podcast_episodes 감정·텍스트 컬럼 추가**

`dev/local_db/mysql/init.sql`의 `podcast_episodes` 테이블에 아래 컬럼 추가 (`trace_id` 컬럼 앞에 삽입):
...
episode_text        LONGTEXT      NULL
                                  COMMENT 'memory_text — segments를 \n\n으로 연결한 전문 (화면 2 본문 표시용)',
```

> ℹ️ `memory_text`는 `script_personalizer.py:197-200`에서 segments의 `script_text`를 `"\n\n"`으로 연결해 생성. AgentState에 저장됨. `_save_core_data()` 시점에 state에서 접근 가능.
...
```python
@pytest.mark.asyncio
async def test_save_core_data_includes_emotion_and_episode_text(mocker):
    """_save_core_data()가 podcast_episodes에 primary_emotion, secondary_emotions, episode_text를 포함해야 한다."""
    ...
    assert saved_data["episode_text"] == "오늘 직장에서 힘드셨군요.\n\n잠시 쉬어가는 시간을 가져보세요."
```
...
episode_data = {
    ...
    "episode_text": final_state.get("memory_text", ""),   # segments \n\n 연결 전문 — 화면 2 본문
}
```

> ℹ️ `memory_text`는 Script Personalizer(`script_personalizer.py:197-200`)에서 TIER 4 처리 시 설정됨. `_save_core_data()`는 TIER 4 완료 후 호출되므로 state에서 접근 가능.
...
git commit -m "feat: init.sql 스키마 수정 (감정 컬럼+episode_text, segments 격리), _save_core_data() 감정·텍스트 필드 확장"
```
