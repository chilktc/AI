...
| ContentAnalyzerAgent `publisher.publish()` | **user_summary 필드만 전송** | Task 9 |
| `content_analyses` 테이블 추가 | init.sql에 테이블 정의 추가 (방안 A 확정) | Task 9 |
...
| ContentAnalyzerAgent | content_analyses | RESOURCE_CONTENT_ANALYSIS | TIER 1 병렬 중 | 비동기 | (내부용, Task 9: user_summary only) | ❌ |
