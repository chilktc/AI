...
| `ingest_mind_frequencies()` 검증 | 백엔드 `mind_frequencies` 테이블과 정합 확인 (현재 코드 호환 확인) | Task 9 |

> ✅ EmotionAgent `publisher.publish()` — **변경 없음 (유지)**. emotion_logs 상세 감정 기록용 유지.
> ✅ `_save_core_data()` 자동 동기 저장 — **변경 없음** (확장만). 파이프라인 완료 후 자동 저장 유지.
> ✅ `ingest_mind_frequencies()` — **코드 수정 없음**. 현재 시그니처(`session_id, keywords: list[str], description: str`)가 백엔드 `mind_frequencies` 테이블과 호환.
...
```python
# [Task 9 추가 — 기존 코드 이후에 추가]
import uuid as _uuid
user_summary = validated_analysis.get("user_summary", {})
await publisher.publish(
    resource=RESOURCE_USER_SUMMARY,   # "user_summaries" — 신규 리소스
    data={
        "summary_id": f"us_{_uuid.uuid4().hex[:12]}",
...
|-------------|------|----------------|------|
| `mind_frequencies` | `id(PK,UUID)`, `session_id(UNIQUE)`, `keywords(String,comma-join)`, `description`, `created_at` | `ingest_mind_frequencies()` | ✅ 현재 호환 |
| `podcasts` | `id(PK,UUID)`, `user_id`, `session_id`, `image_url`, `text`, `created_at` | `ingest_podcast_episodes()` | ⚠️ **Task 8에서 수정** |
...
| **✅ A. 테이블 저장** | `user_summaries` 신규 추가 (화면 1 전용) | `user_id`, `session_id`, `keywords`, `summary` 포함 |
...
| **수정** | `src/agents/podcast/content_analyzer.py` | user_summary 신규 저장 추가 (기존 publisher 유지) + `RESOURCE_USER_SUMMARY` 상수 추가 | Task 9 |
...
| 2 | Task 9 | — | ContentAnalyzer user_summary 신규 저장 추가 (기존 유지) + content_analyses/user_summaries 테이블 + mind_frequencies 검증 |
...
> - **백엔드 podcasts 테이블**: `id(PK,UUID)`, `user_id(백엔드 자동)`, `session_id`, `image_url`, `text`, `created_at(자동)`
...
async def ingest_podcast_episodes(
    ...
    백엔드 컬럼: id(PK,UUID,자동), user_id(자동), session_id, image_url, text, created_at(자동)
    """
...
- Modify: `src/agents/podcast/content_analyzer.py` — user_summary 신규 publisher 호출 추가 (기존 호출 유지)
...
ContentAnalyzerAgent.publisher.publish(user_summary)         ← 신규 추가 (Task 9)
...
**user_summaries:** 신규 publisher 호출로 `user_summary` 두 필드(keywords, summary)만 별도 저장 — 화면 1 전용.
...
CREATE TABLE IF NOT EXISTS user_summaries (
    ...
    FOREIGN KEY (session_id) REFERENCES sessions(session_id) ON DELETE CASCADE,
    FOREIGN KEY (user_id)    REFERENCES users(user_id)    ON DELETE CASCADE,
    INDEX idx_user_summaries_session (user_id, session_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```
...
@pytest.mark.asyncio
async def test_content_analyzer_keeps_existing_publisher():
    """기존 publisher.publish(data=validated_analysis)가 변경 없이 유지되어야 한다."""
    ...
    # 두 번 호출: (1) 기존 전체 저장 (2) user_summary 신규 저장
    assert mock_pub_cls.return_value.publish.call_count == 2
    calls = mock_pub_cls.return_value.publish.call_args_list

    # 첫 번째 호출: 기존 전체 저장 (validated_analysis 전체)
    first_kwargs = calls[0].kwargs
    assert first_kwargs["resource"] == "content_analyses"
    assert "main_theme" in first_kwargs["data"]   # 전체 데이터 포함

    # 두 번째 호출: user_summary 신규 저장
    second_kwargs = calls[1].kwargs
    assert second_kwargs["resource"] == "user_summaries"
    assert set(second_kwargs["data"].keys()) == {"summary_id", "keywords", "summary"}
    assert second_kwargs["data"]["keywords"] == ["번아웃", "스트레스"]
    assert second_kwargs["user_id"] == "u1"
    assert second_kwargs["session_id"] == "s1"
```
...
- [ ] **A-5: backend_resources.py + content_analyzer.py 수정**
...
`src/agents/podcast/content_analyzer.py` — 기존 publisher 유지 + 신규 추가 (L114~121 이후):
...
user_summary = validated_analysis.get("user_summary", {})
await publisher.publish(
    resource=RESOURCE_USER_SUMMARY,
    data={
        "summary_id": f"us_{_uuid.uuid4().hex[:12]}",
        "keywords": user_summary.get("keywords", []),
        "summary": user_summary.get("summary", ""),
...
git commit -m "feat: ContentAnalyzer user_summary 별도 저장 추가 (기존 content_analyses 유지) + user_summaries 테이블"
```

### mind_frequencies 정합 확인 (코드 수정 없음)

> ✅ **현재 코드 호환 확인 완료**: `ingest_mind_frequencies(session_id, keywords: list[str], description: str)` 시그니처가 백엔드 `mind_frequencies` 테이블과 정합.

- [ ] **B-1: 호환성 확인**

```bash
grep -n "ingest_mind_frequencies" src/api/client.py src/agents/podcast/content_analyzer.py
```

확인 항목:
- `client.py`: `(session_id: str, keywords: list[str], description: str)` → 백엔드 `mind_frequencies`(`session_id UNIQUE, keywords String, description`)와 일치
- `content_analyzer.py`: `ingest_mind_frequencies(session_id=..., keywords=sub_themes, description=main_theme)` → fire-and-forget 패턴 유지
- 백엔드: `keywords`는 comma-join String — AI 서버는 `list[str]` 전송, 백엔드가 join 처리 (호환)

> ℹ️ 코드 수정 불필요. 기존 테스트(`tests/api/test_client.py`)의 `ingest_mind_frequencies` 관련 테스트 통과하면 정합 확인 완료.
...
| `ingest_podcast_episodes()` 시그니처 변경 | `(texts, title, summary, keywords)` 제거 → `text: str` | ⚠️ client.py + 호출부 동시 수정 필수 (Task 8) |
...
| `ingest_mind_frequencies()` 호환 | 현재 시그니처가 백엔드 mind_frequencies 테이블과 호환 | ✅ 코드 수정 없음 |
