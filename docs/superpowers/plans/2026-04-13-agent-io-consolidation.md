...
| 항목 | 변경 내용 | Task |
|------|---------|------|
| `podcast_episodes` 테이블 | `script_text`, `tts_markers_json`, `primary_emotion`, `secondary_emotions` 컬럼 추가 — **init.sql 직접 수정** (v3.0 평탄화 반영) | Task 8 |
| `podcast_segments` 테이블 | **init.sql에서 테이블 정의 제거** — v3.0에서 segments 구조 폐기, script_text로 통합 | Task 8 |
| `_save_core_data()` 확장 | emotion_vectors(감정 키워드)를 podcast_metadata 저장 데이터에 추가 (v3.0: 이미 asyncio.gather 패턴, script_text 저장 완료) | Task 8 |
...
> ✅ `PodcastReprocessingAgent` — **메인 파이프라인과 별개**. Frontend 별도 화면에서 사용자 공유 동의 수신 후, Backend → AI서버 `POST /api/episodes/{id}/reprocess` 요청 시에만 실행. `async_post_processing_node`에 **포함하지 않는다**.
...
ContentAnalyzerAgent 완료 (TIER 1)
    → publisher.publish() → BackendClient.save() → POST /api/content_analyses
...
| routes/podcasts.py | podcast_metadata | RESOURCE_PODCAST_METADATA | TIER 4 완료 후 | **동기** | ✅ **화면 2** (v3.0: script_text 저장. Task 8: +감정컬럼, init.sql 정합) |
| routes/podcasts.py | podcast_episodes (ingest) | RESOURCE_PODCAST_EPISODES | TIER 4 완료 후 | **동기** | ✅ mind-frequencies 수집용 (PR #91 신규) |
...
| **에피소드 텍스트** | `script_text` (TEXT) | `final_output.script_text` — v3.0 평탄화된 전체 스크립트 텍스트. `_save_core_data()`에서 이미 저장 중 |
| **이미지** | `cover_image_url` | `visual_data.image_url` |

> ⚠️ **현재 구조 문제 (v3.0 이후)**:
> 1. `script_text`는 `_save_core_data()`에서 `podcast_metadata`에 저장되지만, init.sql의 `podcast_episodes` DDL에는 `script_text` 컬럼이 **아직 없음** → **Task 8에서 추가**
> 2. `memory_text` 생성 코드(`script_personalizer.py:187-188`)가 `.segments` 참조 → v3.0에서 `PersonalizedScript.segments` 제거됨 → **런타임 AttributeError 발생** → **선행 버그 수정 필요 (Task 0)**
> 3. 올바른 수정: `memory_text = personalized_script.script_text` (평탄화된 전체 텍스트 직접 사용)
...
| 2 | Backend → AI서버 재가공 요청 (`POST /api/episodes/{id}/reprocess`) |
...
| 순서 | Task | 의존 | 이유 |
|:----:|:----:|------|------|
| **0** | **Task 0** | — | **선행 필수**: script_personalizer.py `.segments` → `.script_text` 버그 수정 (memory_text 생성) |
...
| 8 | Task 8 | Task 0 | init.sql v3.0 정합 + _save_core_data 감정 컬럼 확장 |
...
**목적:**
1. `podcast_episodes`에 감정 컬럼(`primary_emotion`, `secondary_emotions`) + 텍스트 컬럼(`script_text`, `tts_markers_json`) 추가 — init.sql과 v3.0 코드 정합
2. `podcast_segments` 테이블 **제거** — v3.0에서 segments 구조 폐기, `script_text` 단일 필드로 통합됨
3. `_save_core_data()`에 감정 컬럼만 추가 — v3.0(PR #91)에서 `script_text`/`tts_markers_json`은 이미 저장 중

> ℹ️ **v3.0 현황 (PR #91 이후):**
> - `_save_core_data()`가 이미 `script_text`, `tts_markers_json`을 `podcast_metadata` 리소스로 저장
> - `asyncio.gather(backend_client.save(RESOURCE_PODCAST_METADATA, ...), backend_client.ingest_podcast_episodes(...))` 패턴 적용 완료
> - **미반영 사항**: init.sql DDL에 `script_text`, `tts_markers_json` 컬럼 미추가 + 감정 컬럼 미추가
> - `memory_text`는 `script_text` 직접 사용 (Task 0에서 버그 수정). Pinecone에도 별도 저장 (변경 없음)
...
script_text         TEXT          NULL
                                  COMMENT 'v3.0 평탄화된 전체 스크립트 텍스트 — 화면 2 본문 표시용',
tts_markers_json    TEXT          NULL
                                  COMMENT 'v3.0 TTS 마커 JSON 배열 — [{position, instruction}]',
...
> ℹ️ `script_text`와 `tts_markers_json`은 `_save_core_data()`에서 이미 전송 중 (PR #91). init.sql DDL에 컬럼이 없어 Backend 저장 시 누락될 수 있으므로 추가 필수.
...
>
> ℹ️ **v3.0 현황**: `_save_core_data()`가 이미 `script_text`, `tts_markers_json`을 `RESOURCE_PODCAST_METADATA`로 저장 중. 이 테스트는 감정 컬럼 추가분만 검증한다.
...
from src.api.external_schemas import PodcastEpisodeData, PodcastResponseMeta


def _make_episode_data() -> PodcastEpisodeData:
    """테스트용 PodcastEpisodeData 생성 (v3.0 — script_text 기반)."""
...
async def test_save_core_data_stores_emotion_columns():
    """_save_core_data()가 감정 컬럼(primary_emotion, secondary_emotions)을 저장해야 한다."""
    mock_backend_client = AsyncMock()
    mock_backend_client.save = AsyncMock(return_value=MagicMock(success=True, id="ep_1"))
    mock_backend_client.ingest_podcast_episodes = AsyncMock(return_value=MagicMock(success=True))
    ...
    with patch("src.api.routes.podcasts.backend_client", mock_backend_client):
    ...
    # podcast_metadata 저장 호출 확인 (backend_client.save의 첫 번째 호출)
    save_call = mock_backend_client.save.call_args
    assert save_call is not None, "backend_client.save 호출 없음"
    data = save_call.args[1].data if len(save_call.args) > 1 else save_call.kwargs.get("request", {}).data
    assert data["primary_emotion"] == "anxiety"
    assert data["secondary_emotions"] == ["sadness", "fatigue"]   # [0:2] 슬라이싱
    # script_text는 이미 저장 중 (v3.0) — 확인만
    assert data["script_text"] == "안녕하세요. 오늘 힘든 하루였죠."
```
...
- [ ] **Step 7: _save_core_data() 수정 — 감정 컬럼만 추가**

`src/api/routes/podcasts.py`의 `_save_core_data()` 함수에서 `episode_request = SaveRequest(...)` 블록의 `data={}` 딕셔너리에 감정 컬럼 2개만 추가:

```python
# podcasts.py:160~183 — data={} 블록 안에 추가 (기존 "correlation_id" 뒤)
...
> ℹ️ **v3.0 이후 불필요한 수정:**
> - `script_text`, `tts_markers_json`은 이미 L165-170에서 저장 중 → 변경 없음
> - `segments_data` 구성 코드는 이미 제거됨 (PR #91) → 변경 없음
> - `asyncio.gather` 패턴은 이미 L186-197에서 적용 중 → 변경 없음
...
git commit -m "feat: podcast_segments 제거, podcast_episodes에 script_text/감정 컬럼 추가, _save_core_data() 감정 확장"
```
...
> ℹ️ **PR #91 추가 사항**: ContentAnalyzerAgent가 `publisher.publish()` 외에 `backend_client.ingest_mind_frequencies(session_id, keywords, description)` fire-and-forget 호출도 수행 (content_analyzer.py:125-138). 이 호출은 mind-frequencies 수집용이며, publisher 변경(user_summary only)과는 독립적으로 유지한다.
...
- `POST /api/episodes/{id}/reprocess` 엔드포인트 구현 — PodcastReprocessingAgent 트리거용 FastAPI 라우트 (Backend API 설계 합의 필요)
