...
Ollama(로컬), Anthropic API, AWS Bedrock, OpenAI 모든 프로바이더를 지원한다.
...
사용법:
    ...
    # E2E 커스텀 입력
    python3 -m dev.live_tests.run_live --e2e --input dev/live_tests/test_inputs/my_scenario.json
"""
...
# 지원하는 프로바이더 목록
SUPPORTED_PROVIDERS = ["ollama", "anthropic", "bedrock", "openai"]
...
예시:
  ...
  python3 -m dev.live_tests.run_live --agent content_analyzer --model mistral:7b
    ...
    target.add_argument(
        "--e2e",
        action="store_true",
        help="E2E 멀티 프로바이더 테스트 (Ollama + OpenAI 비교)",
    )
    ...
    parser.add_argument(
        ...
        dest="input_file",
        default=None,
        help="E2E 테스트용 커스텀 입력 JSON 파일 경로",
    )
...
async def run_e2e(input_file: str | None = None) -> None:
    """E2E LangGraph 워크플로우 멀티 프로바이더 테스트를 실행한다."""
    from dev.live_tests.test_e2e_multi_provider import run_all_providers

    await run_all_providers(input_file=input_file)
...
def main() -> None:
    ...
    if args.agent:
        ...
        print("  대상: E2E LangGraph 워크플로우 멀티 프로바이더 테스트")
        asyncio.run(run_e2e(input_file=args.input_file))
